#!/bin/python3
# SYNTAX:
#  python3 update_readme_badge_urls.py <python_versions_file> <pyproject_toml_file> <readme_file>
import sys
import textwrap
import tomllib
...
def render_template(python_versions: list[str], package_version: str) -> list[str]:
    ...
    return [
        ...
        for line in textwrap.dedent(
            f"""
                    ![shields.io](https://img.shields.io/badge/python-{url_python_versions}-blue)
                    ![genbadge](https://bertpl.github.io/counted-float/badges/v{package_version}/tests.svg)
                    ![genbadge](https://bertpl.github.io/counted-float/badges/v{package_version}/coverage.svg)
            """
        ).splitlines()
        if line
    ]
...
def write_to_readme(readme_file: Path, url_lines: list[str]):
    # config
    start_line = "<!--START_SECTION:badges-->"
    end_line = "<!--END_SECTION:badges-->"
...
#  Main entrypoint
# -------------------------------------------------------------------------
def update_readme_badge_urls():
    # --- argument handling -------------------------------
    args = sys.argv[1:]
    if len(args) != 3:
        print(f"expected 3 arguments, got {len(args)}.")
        print(f"Syntax:")
        print(f"  update_readme_badge_urls.py <python_versions_file> <pyproject_toml_file> <readme_file>")
        exit(1)
    else:
