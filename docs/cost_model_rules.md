# Cost-model principles and rules

Every counting decision in `counted-float` answers one question: **what does this operation cost?** The question has more than one defensible answer — the Python code as written, what a compiler would make of it, what the interpreter actually executes — so the model commits to one, through the rules on this page.
...
Everything else follows from that declaration. The model prices your code as an imaginary, competently written C translation — the [compiled port](glossary.md#compiled-port) — in which every plain (non-`CountedFloat`) operand is a compile-time constant, eligible for exactly the optimizations real compilers apply to constants.

Two consequences of the declaration being *by type*:
...
- The contract has an exit to match its entry: **`float(x)` is the explicit opt-out**, returning a plain float that ends counting downstream. Safe because leaving the counted world is the visible point of the call.
...
- converting one **into** the model counts nothing — where an `int` source (bool included) counts the `I2F` its port conversion costs — a stated gap, not an oversight. The same matrix prices every construction route (`CountedFloat(...)`, `from_number`, `fromhex`): int → I2F; float → nothing; `Decimal`, `Fraction` and string parses → nothing; each returns a `CountedFloat`;
