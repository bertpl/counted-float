...
The `MUL` cost is the latency difference between a probe chaining two dependent
`fmul`s per element and one chaining a single `fmul` — probes `f_mul_mul` and
`f_mul`.
...
<!-- BEGIN generated: machine-code-mul-diff -->
```diff
--- f_mul
+++ f_mul_mul
  .L0:
...
+ str  %d1, [%x1], #16
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-mul-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-mul-structure -->
...
??? note "Full ASM listing: `f_mul_mul`"
    ...
    LBB0_10:
      ...
      ret
    ```
<!-- END generated: machine-code-mul-structure -->
