...
The `ACOSH` cost is the latency difference between a probe chaining `math.acosh(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_acosh` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-acosh-diff -->
```diff
--- f_add
+++ f_add_acosh
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-acosh-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-acosh-structure -->
...
??? note "Full ASM listing: `f_add_acosh`"
    ...
    ```
<!-- END generated: machine-code-acosh-structure -->
