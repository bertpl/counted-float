...
The `TANH` cost is the latency difference between a probe chaining `math.tanh(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_tanh` and `f_add`. Like most libm-backed functions,
`math.tanh` compiles to a call into the math library. The probe's small input range keeps the argument clear of `tanh`'s cheap saturate-to-±1 fast path, so the call's real polynomial cost is measured.
...
<!-- BEGIN generated: machine-code-tanh-diff -->
```diff
--- f_add
+++ f_add_tanh
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-tanh-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-tanh-structure -->
...
??? note "Full ASM listing: `f_add_tanh`"
    ...
    ```
<!-- END generated: machine-code-tanh-structure -->
