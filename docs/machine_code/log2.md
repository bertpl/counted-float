...
The `LOG2` cost is the latency difference between a probe chaining `np.log2(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_log2` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-log2-diff -->
```diff
--- f_add
+++ f_add_log2
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-log2-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-log2-structure -->
...
??? note "Full ASM listing: `f_add_log2`"
    ...
    ```
<!-- END generated: machine-code-log2-structure -->
