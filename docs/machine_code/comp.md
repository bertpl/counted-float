...
The `COMP` cost prices a floating-point compare. Its probe, `f_lte_addsub`, chains
...
subtrahend is the *average* of the single-`ADD` and single-`SUB` probes' latencies; the diff
below shows `f_add` (the `SUB` probe differs from it by one opcode only).
...
<!-- BEGIN generated: machine-code-comp-diff -->
```diff
--- f_add
+++ f_lte_addsub
  .L0:
...
+ str  %d2, [%x1], #8
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-comp-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-comp-structure -->
...
??? note "Full ASM listing: `f_lte_addsub`"
    ...
    LBB0_6:
      ...
      ret
    ```
<!-- END generated: machine-code-comp-structure -->
...
3. *Loop-structure symmetry*: `f_add` unrolls 8×, the branchy probe does not; as on the
