...
The `LOG` cost is the latency difference between a probe chaining `log(tmp + x[i])` and one
chaining only `tmp + x[i]` — probes `f_add_log` and `f_add`. Exemplar of a **libm call**: unlike
...
<!-- BEGIN generated: machine-code-log-diff -->
```diff
--- f_add
+++ f_add_log
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-log-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-log-structure -->
...
??? note "Full ASM listing: `f_add_log`"
    ...
    ```
<!-- END generated: machine-code-log-structure -->
