...
The `LGAMMA` cost is the latency difference between a probe chaining
`lgamma(1.5 + 0.5 * sin(tmp + x[i]))` and one chaining only the bounding expression —
probes `f_add_gammabase_lgamma` and `f_add_gammabase`. Chained-base pair: `lgamma`'s
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-lgamma-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-lgamma-structure -->
...
??? note "Full ASM listing: `f_add_gammabase_lgamma`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-lgamma-structure -->
