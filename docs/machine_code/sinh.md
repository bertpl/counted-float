...
The `SINH` cost is the latency difference between a probe chaining
`sinh(asinh(tmp + x[i]))` and one chaining `asinh(tmp + x[i])` — probes `f_add_asinh_sinh`
...
<!-- BEGIN generated: machine-code-sinh-diff -->
```diff
--- f_add_asinh
+++ f_add_asinh_sinh
  .L0:
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-sinh-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-sinh-structure -->
...
??? note "Full ASM listing: `f_add_asinh_sinh`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-sinh-structure -->
