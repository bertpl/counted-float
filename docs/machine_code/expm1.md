...
The `EXPM1` cost is the latency difference between a probe chaining
`expm1(log1p(tmp + x[i]))` and one chaining `log1p(tmp + x[i])` — probes
`f_add_log1p_expm1` and `f_add_log1p`. Chained-base pair: `log1p` is `expm1`'s inverse,
keeping the chain bounded, and the `log1p`-only probe is subtracted so its cost cancels.
...
<!-- BEGIN generated: machine-code-expm1-diff -->
```diff
--- f_add_log1p
+++ f_add_log1p_expm1
  .L0:
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-expm1-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-expm1-structure -->
...
??? note "Full ASM listing: `f_add_log1p_expm1`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-expm1-structure -->
