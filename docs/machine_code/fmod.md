...
The `FMOD` cost is the latency difference between a probe chaining `np.fmod(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_fmod` and `f_add`. Like most libm-backed functions,
`np.fmod` compiles to a call into the math library. The probe's strictly positive divisor range avoids the `fmod(x, 0)` domain error.
...
<!-- BEGIN generated: machine-code-fmod-diff -->
```diff
--- f_add
+++ f_add_fmod
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-fmod-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-fmod-structure -->
...
??? note "Full ASM listing: `f_add_fmod`"
    ...
    ```
<!-- END generated: machine-code-fmod-structure -->
