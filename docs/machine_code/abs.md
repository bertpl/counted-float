...
The `ABS` cost is the latency difference between a probe chaining `abs(tmp + x[i])` and one
chaining only `tmp + x[i]` — probes `f_add_abs` and `f_add`. On ARM64 this compiles to
...
<!-- BEGIN generated: machine-code-abs-diff -->
```diff
--- f_add
+++ f_add_abs
  .L0:
...
+ fabs  %d1, %d1
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-abs-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-abs-structure -->
...
??? note "Full ASM listing: `f_add_abs`"
    ...
    LBB0_6:
      ...
      ret
    ```
<!-- END generated: machine-code-abs-structure -->
