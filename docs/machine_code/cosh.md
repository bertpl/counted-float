...
The `COSH` cost is the latency difference between a probe chaining
`cosh(acosh(tmp + x[i]))` and one chaining `acosh(tmp + x[i])` — probes `f_add_acosh_cosh`
...
<!-- BEGIN generated: machine-code-cosh-diff -->
```diff
--- f_add_acosh
+++ f_add_acosh_cosh
  .L0:
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-cosh-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-cosh-structure -->
...
??? note "Full ASM listing: `f_add_acosh_cosh`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-cosh-structure -->
