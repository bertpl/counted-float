...
The `ADD` cost is the latency difference between a probe chaining two dependent
`fadd`s per element and one chaining a single `fadd` — probes `f_add_add` and
`f_add`.
...
<!-- BEGIN generated: machine-code-add-diff -->
```diff
--- f_add
+++ f_add_add
  .L0:
...
+ str  %d1, [%x1], #16
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-add-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-add-structure -->
...
??? note "Full ASM listing: `f_add_add`"
    ...
    LBB0_10:
      ...
      ret
    ```
<!-- END generated: machine-code-add-structure -->
