...
The `LOG1P` cost is the latency difference between a probe chaining `math.log1p(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_log1p` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-log1p-diff -->
```diff
--- f_add
+++ f_add_log1p
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-log1p-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-log1p-structure -->
...
??? note "Full ASM listing: `f_add_log1p`"
    ...
    ```
<!-- END generated: machine-code-log1p-structure -->
