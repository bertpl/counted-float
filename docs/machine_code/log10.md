...
The `LOG10` cost is the latency difference between a probe chaining `np.log10(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_log10` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-log10-diff -->
```diff
--- f_add
+++ f_add_log10
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-log10-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-log10-structure -->
...
??? note "Full ASM listing: `f_add_log10`"
    ...
    ```
<!-- END generated: machine-code-log10-structure -->
