...
The `COS` cost is the latency difference between a probe chaining `math.cos(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_cos` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-cos-diff -->
```diff
--- f_add
+++ f_add_cos
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-cos-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-cos-structure -->
...
??? note "Full ASM listing: `f_add_cos`"
    ...
    ```
<!-- END generated: machine-code-cos-structure -->
