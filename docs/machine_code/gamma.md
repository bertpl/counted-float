...
The `GAMMA` cost is the latency difference between a probe chaining
`gamma(1.5 + 0.5 * sin(tmp + x[i]))` and one chaining only the bounding expression —
probes `f_add_gammabase_gamma` and `f_add_gammabase`. Chained-base pair: `gamma` has no
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-gamma-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-gamma-structure -->
...
??? note "Full ASM listing: `f_add_gammabase_gamma`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-gamma-structure -->
