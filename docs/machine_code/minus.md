...
The `MINUS` cost is the latency difference between a probe chaining `-(tmp + x[i])` and one
chaining only `tmp + x[i]` — probes `f_add_minus` and `f_add`. On ARM64 this compiles to
...
<!-- BEGIN generated: machine-code-minus-diff -->
```diff
--- f_add
+++ f_add_minus
  .L0:
...
+ fneg  %d1, %d1
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-minus-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-minus-structure -->
...
??? note "Full ASM listing: `f_add_minus`"
    ...
    LBB0_6:
      ...
      ret
    ```
<!-- END generated: machine-code-minus-structure -->
