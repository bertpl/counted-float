...
The `ACOS` cost is the latency difference between a probe chaining `acos(sin(tmp + x[i]))`
and one chaining `sin(tmp + x[i])` — probes `f_add_sin_acos` and `f_add_sin`. Chained-base
...
<!-- BEGIN generated: machine-code-acos-diff -->
```diff
--- f_add_sin
+++ f_add_sin_acos
  .L0:
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-acos-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-acos-structure -->
...
??? note "Full ASM listing: `f_add_sin_acos`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-acos-structure -->
