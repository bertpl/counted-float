...
The `EXP2` cost is the latency difference between a probe chaining
`exp2(log2(tmp + x[i]))` and one chaining `log2(tmp + x[i])` — probes `f_add_log2_exp2`
and `f_add_log2`. Chained-base pair: `log2` bounds the argument (mirroring the
[EXP](exp.md) construction), and the `log2`-only probe is subtracted so its cost cancels.
...
<!-- BEGIN generated: machine-code-exp2-diff -->
```diff
--- f_add_log2
+++ f_add_log2_exp2
  .L0:
...
+ subs  %x4, %x4, #1
  b.ne  .L0
```
<!-- END generated: machine-code-exp2-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-exp2-structure -->
...
??? note "Full ASM listing: `f_add_log2_exp2`"
    ...
    LBB0_6:
      ...
      .loh AdrpLdrGot  Lloh0, Lloh1
    ```
<!-- END generated: machine-code-exp2-structure -->
