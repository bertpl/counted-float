...
The `TAN` cost is the latency difference between a probe chaining `math.tan(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_tan` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-tan-diff -->
```diff
--- f_add
+++ f_add_tan
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-tan-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-tan-structure -->
...
??? note "Full ASM listing: `f_add_tan`"
    ...
    ```
<!-- END generated: machine-code-tan-structure -->
