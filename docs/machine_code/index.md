...
Every benchmarked flop weight is a measured latency *difference* between two compiled probes: a
probe containing the operation of interest, minus a probe identical except for that operation.
...
two probes as a unified diff, each probe's loop structure, and the complete compiled functions.
...
1. **What the diff isolates** — the intended instruction (or call), and whatever else changed
   with it.
2. **Where it sits in the dependency chain** the probe serializes through — the measurement is
...
<!-- BEGIN generated: machine-code-page-list -->
...
- **Architecture: ARM64 (Apple M-series).** Machine code is architecture-specific, so the
  ...
  the probe pair, and that shows on any single target.
- **The diff shows inner loops.** Full-function dumps are dominated by prologue and setup code
  ...
  two probes' compiled native functions. The compiler may turn one source loop into several
  innermost loops (e.g. an 8×-unrolled main loop plus a scalar remainder loop) — each page
  inventories all of them, and the diff shows the best-matching pair across the two probes. The
...
- **Registers and labels are canonicalized** (renamed in order of first appearance, e.g. `%x0`,
  ...
  register allocator picking different names for the same code. One side effect: when one probe
...
or fallback — are stated in [Cost-model principles](../cost_model_rules.md).
