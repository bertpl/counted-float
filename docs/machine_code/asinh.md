...
The `ASINH` cost is the latency difference between a probe chaining `math.asinh(tmp + x[i])` and
one chaining only `tmp + x[i]` — probes `f_add_asinh` and `f_add`. Like most libm-backed functions,
...
<!-- BEGIN generated: machine-code-asinh-diff -->
```diff
--- f_add
+++ f_add_asinh
  .L0:
...
+ subs  %x3, %x3, #1
  b.ne  .L0
```
<!-- END generated: machine-code-asinh-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-asinh-structure -->
...
??? note "Full ASM listing: `f_add_asinh`"
    ...
    ```
<!-- END generated: machine-code-asinh-structure -->
