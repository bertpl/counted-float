...
The `RND` cost is the latency difference between a probe chaining `np.round(tmp + x[i])` and one
chaining only `tmp + x[i]` — probes `f_add_round` and `f_add`. On ARM64 this compiles to
...
<!-- BEGIN generated: machine-code-rnd-diff -->
```diff
--- f_add
+++ f_add_round
  .L0:
...
+ frintx  %d1, %d1
  ...
  b.ne  .L0
```
<!-- END generated: machine-code-rnd-diff -->

## Loop structure

<!-- BEGIN generated: machine-code-rnd-structure -->
...
??? note "Full ASM listing: `f_add_round`"
    ...
    LBB0_6:
      ...
      .cfi_endproc
    ```
<!-- END generated: machine-code-rnd-structure -->
