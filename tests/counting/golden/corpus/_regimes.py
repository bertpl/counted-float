...
ROWS += [
    *row(
        ...
        requires="numpy",
        twin=False,
        probe=_numpy_probe(lambda num, np: num(2.0) * np.array([1.0])),
    ),
    *row(
        "A14",
        "num(2.0) * np.float32(1.0)",
        {},
        None,
        R_SCOPE,
        raises=TypeError,
        requires="numpy",
        twin=False,
        probe=_numpy_probe(lambda num, np: num(2.0) * np.float32(1.0)),
    ),
    *row(
        "A14",
        "(num(1.0) + np.float64(2.0), np.float64(2.0) + num(1.0))",
        {"ADD": 2},
        (CF, CF),
        R_SCOPE,
        requires="numpy",
        twin=False,
        probe=_numpy_probe(lambda num, np: (num(1.0) + np.float64(2.0), np.float64(2.0) + num(1.0))),
    ),
]
